...
docker build -t ${DOCKER_PREFIX}portal-env:${DOCKER_TAG}-onbuild . >> docker-portal.log 
popd
...
docker build -t ${DOCKER_PREFIX}portal-api:${DOCKER_TAG} . >> docker-portal.log
popd

pushd ../wicked.portal
echo Building Portal docker image...
perl -pe 's;(\\*)(\$([a-zA-Z_][a-zA-Z_0-9]*)|\$\{([a-zA-Z_][a-zA-Z_0-9]*)\})?;substr($1,0,int(length($1)/2)).($2&&length($1)%2?$2:$ENV{$3||$4});eg' Dockerfile.template > Dockerfile
docker build -t ${DOCKER_PREFIX}portal:${DOCKER_TAG} . >> docker-portal.log
popd
...
docker-compose -f local-compose-portal.yml build >> docker-portal.log
...
docker-compose -f local-compose-portal.yml down >> docker-portal.log
